...
router.get("/category/:name", async (req, res) => {
  ...
  const categoryAds = await Ad.paginate(
...
router.get("/", async (req, res) => {
  ...
  if (req.query.search) {
    ...
    let resultText;
    if (adsCount == 1) resultText = "ogłoszenie";
    else if (adsCount >= 2 && adsCount <= 4) resultText = "ogłoszenia";
    else if (adsCount >= 5 || adsCount == 0) resultText = "ogłoszeń";
...
router.get(
  ...
  catchAsync(async (req, res) => {
    ...
    if (!ad.author.equals(req.user._id) && req.user.isAdmin == false) {
      req.flash(
        "error",
        "Nie masz uprawnień do zaktualizowania tego ogłoszenia!"
      );
...
router.put(
  ...
  catchAsync(async (req, res) => {
    ...
    const adNew = await Ad.findByIdAndUpdate(req.params.id, {
      title: req.body.ad.title,
      price: req.body.ad.price,
      category: categoryNew._id,
      description: req.body.ad.description,
      contactNumber: req.body.ad.contactNumber,
